...
var REVIEW_1_DELAY = 30 * 1e3;
var REVIEW_2_DELAY = 60 * 1e3;
var cards = [];
var progress = {};
var currentCard = null;
var listening = false;
var recognition = null;
var sessionSkippedCards = /* @__PURE__ */ new Set();
var wrongAttempts = 0;
var MAX_WRONG_ATTEMPTS = 3;
var currentSet = "body-parts";
var homeScreen = document.getElementById("homeScreen");
var learnScreen = document.getElementById("learnScreen");
var analyticsScreen = document.getElementById("analyticsScreen");
...
var analyticsBackBtn = document.getElementById("analyticsBackBtn");
var wordEl = document.getElementById("text");
var cardStatusEl = document.getElementById("cardStatus");
var progressFill = document.getElementById("progressFill");
var progressText = document.getElementById("progressText");
var nextReviewTime = document.getElementById("nextReviewTime");
...
var setupProgressBtn = document.getElementById("setupProgressBtn");
var progressSetupScreen = document.getElementById("progressSetupScreen");
var closeProgressSetupBtn = document.getElementById("closeProgressSetupBtn");
var markAllNewBtn = document.getElementById("markAllNewBtn");
var markAllLearningBtn = document.getElementById("markAllLearningBtn");
var markAllLearnedBtn = document.getElementById("markAllLearnedBtn");
var progressSearchInput = document.getElementById("progressSearchInput");
var progressCardList = document.getElementById("progressCardList");
var exportProgressBtn = document.getElementById("exportProgressBtn");
var importProgressBtn = document.getElementById("importProgressBtn");
var importProgressFile = document.getElementById("importProgressFile");
var exportCardsetBtn = document.getElementById("exportCardsetBtn");
var importCardsetBtn = document.getElementById("importCardsetBtn");
var deleteCardsetBtn = document.getElementById("deleteCardsetBtn");
var importCardsetFile = document.getElementById("importCardsetFile");
...
var themeOptionInputs = document.querySelectorAll('input[name="themeOption"]');
var deleteDatabaseBtn = document.getElementById("deleteDatabaseBtn");
var confirmSetupProgressBtn = document.getElementById("confirmSetupProgressBtn");
...
var LATEST_DATA_VERSION = 1;
var DB_NAME = "flashCardDB";
var DB_VERSION = 1;
var CARDSETS_STORE_NAME = "cardsets";
var CARDSET_METADATA_STORE_NAME = "cardsetMetadata";
var PROGRESS_STORE_NAME = "progress";
var SETTINGS_STORE_NAME = "settings";
var CACHE_VERSION_KEY = "cached_data_version";
var CSV_DELIMITER = ",";
var dbConnection = null;
var DEFAULT_CARDSETS = [
...
async function fetchAndSeed(currentSet2, db) {
  let data;
  try {
    const { response, url } = await resolveCardsetUrl(currentSet2);
    let text;
    if (url.endsWith(".gz")) {
    ...
    throw new Error(`Failed to fetch cardset for '${currentSet2}': ${error.message}`);
  }
  return new Promise((resolve, reject) => {
    ...
    try {
      for (const item of data) {
        ...
        store.put({ ...item, setName: currentSet2 });
      }
    } catch (error) {
      reject(error);
    }
...
function getAllCardsForSet(currentSet2) {
  return openDatabase().then((db) => {
    return new Promise((resolve, reject) => {
      ...
      if (store.indexNames.contains("setName")) {
        const index = store.index("setName");
        request = index.openCursor(IDBKeyRange.only(currentSet2));
      } else {
      ...
      request.onsuccess = (event) => {
        ...
        const record = cursor.value;
        if (!record.setName || record.setName === currentSet2) {
...
function getDisplayNameForSet(setName) {
  ...
  return setName.replace(/[-_]/g, " ").replace(/\b\w/g, (char) => char.toUpperCase());
}
...
function getCardsetDisplayName(setName) {
  return getCardsetMetadata(setName).then((metadata) => metadata?.displayName || getDisplayNameForSet(setName));
}
...
function updateDeleteCardsetButtonState() {
  ...
  deleteCardsetBtn.title = isDefault ? "Default cardsets cannot be deleted" : "Delete the selected cardset";
}
function deleteCardsetRecords(db, setName) {
  return new Promise((resolve, reject) => {
    const tx = db.transaction(CARDSETS_STORE_NAME, "readwrite");
    const store = tx.objectStore(CARDSETS_STORE_NAME);
    const request = store.indexNames.contains("setName") ? store.index("setName").openCursor(IDBKeyRange.only(setName)) : store.openCursor();
...
async function importCardset(setName, cards2) {
  const db = await openDatabase();
  await deleteCardsetRecords(db, setName);
  return new Promise((resolve, reject) => {
    const tx = db.transaction(CARDSETS_STORE_NAME, "readwrite");
...
function createCsvFromCards(cards2) {
...
function createCsvFromProgress(progressData, cards2) {
...
async function exportProgress() {
  try {
    ...
    link.href = url;
    link.download = `${currentSet}-progress-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv`;
...
async function exportCardset() {
  try {
    const cards2 = await getAllCardsForSet(currentSet);
    if (!cards2.length) {
      alert("Nothing to export for this cardset.");
      return;
    }
    const csvText = createCsvFromCards(cards2.map((card) => {
...
async function initializeCardSet(currentSet2) {
  try {
    ensureStorageAvailable();
    const db = await openDatabase();
    const cachedVersion = await getCachedDataVersion();
    const setCount = await getStoreCountForSet(db, currentSet2);
    if (cachedVersion !== LATEST_DATA_VERSION) {
      await clearDefaultCardsets(db);
      const isDefault = DEFAULT_CARDSETS.some((item) => item.key === currentSet2);
      if (isDefault) {
        await fetchAndSeed(currentSet2, db);
      }
      return await getAllCardsForSet(currentSet2);
    }
    if (setCount > 0) {
      return await getAllCardsForSet(currentSet2);
    }
...
function getStageName(stage) {
  switch (stage) {
    case 0:
      return "\u{1F195} New";
    case 1:
      return "\u{1F501} Repeat x1";
    case 2:
      return "\u{1F501} Repeat x2";
    case 3:
      return "\u2705 Learned";
    default:
      return "Unknown";
...
function showCurrentCard() {
  ...
  wrongAttempts = 0;
  const p = getCardProgress(
    currentCard.text
  );
  wordEl.textContent = currentCard.text;
  cardStatusEl.textContent = getStageName(
    p.stage
  );
...
function updateAttemptDisplay() {
  ...
  const statusText = attemptsLeft > 0 ? `${attemptsLeft} attempt${attemptsLeft === 1 ? "" : "s"} left` : "No attempts left";
  recognizedText.textContent = listening ? `\u{1F3A4} Listening... (${statusText})` : `Press Start Listening \u2014 ${statusText}`;
}
...
function markWrong(card) {
  ...
  if (wrongAttempts >= MAX_WRONG_ATTEMPTS) {
    const answerText = Array.isArray(card.answers) ? card.answers.join(", ") : String(card.answers || card.text);
...
function skipCurrentCard() {
  ...
  saveProgress();
  updateHomeStats();
  const answerText = Array.isArray(currentCard.answers) ? currentCard.answers.join(", ") : String(currentCard.answers || currentCard.text);
...
function showWaitingScreen() {
  ...
  wordEl.textContent = "\u{1F389} Great job!";
...
function updateWaitingDisplay() {
  ...
  if (nextTime === null) {
    recognizedText.textContent = "All cards learned \u{1F389}";
    clearInterval(
      waitingTimer
    );
    return;
...
function checkAnswer(text) {
  ...
  const answers = currentCard.answers.map(
    normalize
  );
  if (answers.includes(
    spoken
...
function stopListening() {
  ...
  listening = false;
  listenBtn.textContent = "\u{1F3A4} Start Listening";
...
function buildAnalytics() {
  document.getElementById(
    "analyticsLearned"
  ).textContent = getLearnedCount();
  document.getElementById(
    "analyticsStage1"
  ).textContent = getReview1Count();
  document.getElementById(
    "analyticsStage2"
  ).textContent = getReview2Count();
  document.getElementById(
    "analyticsNew"
  ).textContent = getNewCount();
  const tbody = document.querySelector(
    "#analyticsTable tbody"
  );
  tbody.innerHTML = "";
  const sortedCards = [...cards].sort(
    (a, b) => a.text.localeCompare(
      b.text
    )
  );
  for (const card of sortedCards) {
    ...
    const wordCell = document.createElement(
      "td"
    );
    ...
    switch (p.stage) {
      case 0:
        statusText = "\u{1F195} New";
        className = "status-new";
        break;
      case 1:
        statusText = "\u{1F501} Repeat x1";
        className = "status-review1";
        break;
      case 2:
        statusText = "\u{1F501} Repeat x2";
    ...
    const answerCell = document.createElement(
      "td"
    );
    answerCell.textContent = Array.isArray(card.answers) ? card.answers.join(", ") : String(card.answers || "");
    row.appendChild(
      answerCell
    );
    tbody.appendChild(
...
setInterval(
  () => {
    if (homeScreen.classList.contains(
      "active"
    )) {
      updateNextReviewLabel();
    }
...
setInterval(
  () => {
    if (!learnScreen.classList.contains(
      "active"
    )) {
      return;
    }
...
var progressBackup = null;
var progressSetupPreviousScreen = analyticsScreen;
function openProgressSetupScreen() {
  progressBackup = JSON.parse(
...
function renderProgressCardList(searchTerm = "") {
  const filteredCards = cards.filter(
    (card) => card.text.toLowerCase().includes(
  ...
  for (const card of filteredCards) {
    ...
    statusDiv.textContent = getStageName(p.stage) + " (Correct: " + p.correctCount + ")";
    ...
    const stages = [
      { value: 0, label: "\u{1F195} New" },
      { value: 1, label: "\u{1F501} Repeat x1" },
      { value: 2, label: "\u{1F501} Repeat x2" },
